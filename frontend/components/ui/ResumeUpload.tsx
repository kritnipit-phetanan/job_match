...
export default function ResumeUpload() {
  ...
  // State สำหรับ Skill Gap
  const [skillGaps, setSkillGaps] = useState<Record<number, { matched: string[], missing: string[] }>>({});
  const [analyzingGapId, setAnalyzingGapId] = useState<number | null>(null);
  ...
  const handleGenerateCoverLetter = async (job: any) => {
    ...
    try {
      ...
      setCoverLetters(prev => ({ ...prev, [job.id]: "❌ ขออภัย ไม่สามารถสร้างจดหมายได้ในขณะนี้" }));
  ...
  const handleAnalyzeGap = async (job: any) => {
    ...
    try {
      ...
      alert("ขออภัย ไม่สามารถวิเคราะห์ Skill Gap ได้ในขณะนี้");
    } finally {
                  ...
                  <div className="flex flex-col md:flex-row justify-between gap-6">
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="flex flex-col gap-3 mt-4">
                        {skillGaps[job.id] ? (
                          // แสดงผลเมื่อวิเคราะห์ Gap เสร็จแล้ว
                          <div className="flex flex-col gap-2 animate-in fade-in zoom-in duration-300">
                            {skillGaps[job.id].matched.length > 0 && (
                              <div className="flex flex-wrap gap-2 items-center">
                                <span className="text-sm font-semibold text-green-600 w-16">✅ มี:</span>
                                {skillGaps[job.id].matched.map((skill, idx) => (
                                  <Badge key={`m-${idx}`} className="bg-green-100 text-green-800 hover:bg-green-200 border-green-200 font-medium">
                                    {skill}
                                  </Badge>
                                ))}
                              </div>
                            )}
                            {skillGaps[job.id].missing.length > 0 && (
                              <div className="flex flex-wrap gap-2 items-center">
                                <span className="text-sm font-semibold text-red-600 w-16">❌ ขาด:</span>
                                {skillGaps[job.id].missing.map((skill, idx) => (
                                  <Badge key={`x-${idx}`} variant="outline" className="bg-red-50 text-red-600 hover:bg-red-100 border-red-200 font-medium">
                                    {skill}
                                  </Badge>
                                ))}
                              </div>
                          ...
                          // แสดงผลแบบ Default (ยังไม่ได้วิเคราะห์)
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {/* ปุ่มกดให้ AI เริ่มวิเคราะห์ */}
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="h-6 px-2 text-xs text-primary font-semibold hover:bg-primary/10 ml-2"
                              onClick={() => handleAnalyzeGap(job)}
                              disabled={analyzingGapId === job.id}
                            >
                              {analyzingGapId === job.id ? <Loader2 className="w-3 h-3 animate-spin mr-1"/> : "🔍 วิเคราะห์ Gap"}
                            </Button>
                          </div>
                        )}
                      </div>
